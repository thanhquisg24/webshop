...
public interface ProductImageRepository extends JpaRepository<ProductImage, Long> {
	...
	@Query("select p from ProductImage p inner join fetch p.product pp where p.id = ?1")
	ProductImage findOne(Long id);

	@Query("select p from ProductImage p inner join fetch p.product pp where pp.id = ?1")
