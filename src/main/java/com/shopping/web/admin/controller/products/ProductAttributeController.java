...
import com.shopping.core.business.utils.ProductPriceUtils;
...
//	@PreAuthorize("hasRole('PRODUCTS')")
	@RequestMapping(value="/admin/products/attributes/list", method=RequestMethod.GET)
	@ResponseBody
	public JsonResponse<ProductAttribute> displayProductAttributes(@RequestParam("id") Long productId) throws ServiceException  {
		
		List<ProductAttribute> result=productAttributeService.getByProductId(productId);
		if(result==null){
			result=new ArrayList<ProductAttribute>();
		}
		return new JsonResponse<ProductAttribute>(result) ;
...
/*	private String displayAttribute(Long productId, Long id, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	*/
	//@PreAuthorize("hasRole('PRODUCTS')")
	/*@RequestMapping(value="/admin/attributes/attribute/save", method=RequestMethod.POST)
